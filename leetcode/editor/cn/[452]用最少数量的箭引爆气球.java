...
class Solution {
    public int findMinArrowShots(int[][] points) {
        ...
        if (points.length == 0)
            return numx;
        int lastnum = points[0][1];           //上一支箭的坐标
        int nextnum = 0;                      //下一支箭的坐标
        boolean flag = false;                 //判断下一只箭是否赋值
        for (int j = 0; j < points.length; j++) {
            if (numx == 0) {
                for (int i = 0; i < points.length; i++) {
                    if (points[i][1] < lastnum)
                        lastnum = points[i][1];
                }
                numx++;
            } else {
                for (int i = 0; i < points.length; i++) {
                    if (points[i][0] > lastnum && !flag) {
                        nextnum = points[i][1];
                        flag = true;
                    }
                    if (points[i][0] > lastnum && points[i][1] < nextnum && flag) {
                        nextnum = points[i][1];
                    }
                }
                if (!flag)
